...
from ui.components.config_componentes import BOTTOM_ALTURA_CAIXA, BOTTOM_OFFSET_AREA_DESENHO, BOTTOM_OFFSET_CLIPPING_Y, BOTTOM_ALTURA_UTIL_DIFF, BOTTOM_MARGIN_X, CONFIG_OFFSET_Y_INTERNO, ESTUDOS_OFFSET_Y_INTERNO, ESTUDOS_DESC_OFFSET_X, BOTTOM_SUBABA_Y_OFFSET, BOTTOM_SUBABA_ALTURA, BOTTOM_SUBABA_MARGIN_X
                    ...
                    for idx_mod, modulo in enumerate(lista_ativa):
                        if modulo.estado == 'painel':
                            if not hasattr(modulo, 'escala_atual_painel') or modulo.escala_atual_painel != escala_compacta:
                                # Usamos sempre a imagem base (escala 1.0) para redimensionar com precisão
                                surf_orig = getattr(modulo, 'imagem_braco', modulo.imagem_painel)
                                w_novo = int(surf_orig.get_width() * escala_compacta)
                                h_novo = int(surf_orig.get_height() * escala_compacta)
