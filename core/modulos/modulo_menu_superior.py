...
class MenuSuperior:
    ...
    def desenhar(self, tela, fonte_ui, estado=None):
        ...
        for menu_chave, rect in self.rects_principais.items():
            ...
            cor_fundo = self.cor_hover if self.item_hover == menu_chave or self.menu_aberto == menu_chave else (0,0,0,0) # Transparente se não houver hover
            if cor_fundo != (0,0,0,0):
                pygame.draw.rect(tela, cor_fundo, rect)
            txt = fonte_ui.render(titulo_exibir, True, self.cor_texto)
            tela.blit(txt, (rect.x + (rect.width // 2 - txt.get_width() // 2), rect.y + (rect.height // 2 - txt.get_height() // 2)))
