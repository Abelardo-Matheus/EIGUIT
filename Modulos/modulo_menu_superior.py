...
class MenuSuperior:
    def __init__(self):
        ...
        # Estrutura base sempre em Português para servir de chave
        self.ordem_menus = ["Arquivo", "Perfil", "Configurações", "Ajuda"]
        self.estrutura = {
            "Arquivo": ["Novo", "Abrir", "Salvar", "Exportar"],
            "Perfil": ["Minha Conta (Cloud)", "Deslogar (Trocar Conta)", "Criar Novo Perfil", "Carregar Perfil", "Deletar Perfil Atual", "Voltar para o Padrão", "Imprimir", "Sair"],
    ...
    def executar_acao(self, acao, estado, configs, campo, gravador):
        if acao == "Sair": estado.solicitou_saida = True
            ...
            if hasattr(estado, 'gerenciador_perfil'): estado.gerenciador_perfil.abrir_modal_conta(estado)
        ...
        elif acao == "Suporte": self.gerenciador_suporte.aberto = True
    ...
    def desenhar_modal_ideias(self, tela, fonte_ui, estado):
        ...
        lines = [
            _t("Grandes ferramentas não nascem no isolamento; elas ganham vida através"),
            _t("do diálogo direto com quem as utiliza. Se você vislumbrou um recurso,"),
            _t("identificou falhas ou quer sugerir refinamentos técnicos, sua visão é crucial."),
            _t("O EIGUIT pertence à comunidade, e Pull Requests são muito bem-vindos.")
        ]
    ...
    def desenhar_modal_patrocine(self, tela, fonte_ui, estado=None):
        ...
        lines = [
            _t("Manter uma plataforma de código aberto exige dedicação, estudo e infraestrutura."),
            _t("Se o Guitar Studio IA trouxe clareza ou impulsionou sua rotina de estudos,"),
            _t("saiba que seu incentivo é o que viabiliza a evolução contínua do sistema."),
            _t("Por enquanto, o suporte ao projeto é centralizado de forma direta via PIX.")
        ]
        for i, line in enumerate(lines):
            txt = fonte_ui.render(line, True, (200, 200, 200))
            tela.blit(txt, (centro_x_modal - txt.get_width() // 2, cy + 85 + i*30))
    ...
    def desenhar_modal_motivacao(self, tela, fonte_ui, estado=None):
        ...
        lines = [
            _t("O EIGUIT nasceu de uma profunda inquietação pessoal. Diante dos labirintos"),
            _t("teóricos e da fragmentação de materiais que frequentemente frustram o estudo"),
            _t("da guitarra, idealizei este projeto, inicialmente, como um utilitário de uso restrito"),
            _t("— um porto seguro para mapear escalas e visualizar intervalos de forma ágil."),
            _t("Contudo, à medida que as linhas de código se fundiam com as necessidades musicais,"),
            _t("o software expandiu-se a ponto de se tornar um ambiente completo de prática."),
            _t("Compreendi, então, que reter essa ferramenta seria privar outros músicos do mesmo amparo."),
            _t("É uma honra abrir este ecossistema para que novos entusiastas aprimorem sua técnica"),
    ...
    def desenhar(self, tela, fonte_ui, estado=None):
        ...
        for menu_chave, rect in self.rects_principais.items():
            # Tradução dinâmica do título do menu
        ...
        if self.menu_aberto:
            ...
            for i, texto_chave in enumerate(itens_sub):
                ...
                if self.sub_item_hover == i: pygame.draw.rect(tela, self.cor_hover, rect_item)
                
                # Tradução dinâmica do item do menu
        ...
        if self.modal_ideias_aberto: self.desenhar_modal_ideias(tela, fonte_ui, estado)
        elif self.modal_patrocine_aberto: self.desenhar_modal_patrocine(tela, fonte_ui, estado)
        elif self.modal_motivacao_aberto: self.desenhar_modal_motivacao(tela, fonte_ui, estado)
        elif self.gerenciador_suporte.aberto: self.gerenciador_suporte.desenhar(tela, fonte_ui, pygame.font.SysFont(None, 24, bold=True), estado)
