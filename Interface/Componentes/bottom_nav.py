...
from Interface.Componentes.config_componentes import (
    BOTTOM_ALTURA_CAIXA, BOTTOM_OFFSET_AREA_DESENHO, BOTTOM_OFFSET_CLIPPING_Y,
    BOTTOM_ALTURA_UTIL_DIFF, BOTTOM_MARGIN_X, CONFIG_OFFSET_Y_INTERNO,
    ESTUDOS_OFFSET_Y_INTERNO, ESTUDOS_DESC_OFFSET_X,
    BOTTOM_SUBABA_Y_OFFSET, BOTTOM_SUBABA_ALTURA, BOTTOM_SUBABA_MARGIN_X
)

def quebrar_texto(texto, fonte, max_largura):
    """Auxiliar para quebra de linha (Word Wrap) no Pygame."""
...
def _desenhar_aba_configuracao(tela, dx, y_start, largura_conteudo, estado, fontes, configs, meu_metronomo, memoria_sub_aba):
    if memoria_sub_aba == 0:
        ...
        if hasattr(meu_metronomo, 'x_config'): meu_metronomo.x_config = dx + BOTTOM_MARGIN_X
...
def _desenhar_aba_estudos(tela, dx, y_start, largura_conteudo, estado, fontes, memoria_sub_aba):
    ...
    # Cálculo da largura disponível para o texto (Resolução Nova)
    # Tiramos o botão (170) e as margens (60)
    ...
    if memoria_sub_aba == 0:
        ...
        for titulo_btn, descricao in textos:
            ...
            # Aplica Word Wrap Dinâmico na Descrição
...
def _desenhar_aba_musicas(tela, dx, y_start, largura_conteudo, estado, fontes, memoria_sub_aba):
    if memoria_sub_aba == 0:
        # Mini barra de navegação interna: Pesquisa | Favoritas | Minhas Músicas
        ...
        if estado.sub_memoria_musicas == 0:
            pygame.draw.rect(tela, (0, 120, 215), rect_aba_busca, border_radius=15)
        elif estado.sub_memoria_musicas == 1:
            pygame.draw.rect(tela, (0, 120, 215), rect_aba_favs, border_radius=15)
        else:
            pygame.draw.rect(tela, (0, 120, 215), rect_aba_locais, border_radius=15)
        ...
        if estado.sub_memoria_musicas == 0:
            ...
            cor_borda_busca = (0, 163, 255) if getattr(estado, 'songsterr_search_active', False) else (100, 100, 100)
            ...
            if estado.songsterr.carregando:
                 ...
                 for i, song in enumerate(estado.resultados_songsterr[:5]): # Mostra top 5
                      y_item = y_lista + (i * 45)
                      ...
                      if not hasattr(estado, 'rects_favoritos_click'): estado.rects_favoritos_click = []
            ...
            if not estado.favoritos_songsterr:
                ...
                estado.rects_resultados_songsterr = [] # Reutiliza o mesmo sistema de clique
            ...
            # --- TELA DE MINHAS MÚSICAS (LOCAL MIDI) ---
            y_lista = y_start + 50
            
            # Botão Adicionar MIDI
            rect_add = pygame.Rect(dx + 20, y_lista, 200, 35)
            pygame.draw.rect(tela, (0, 163, 255), rect_add, border_radius=8)
...
def desenhar_secoes_inferiores_expansiveis(tela, estado, configs, dicionario_escalas, fontes, meu_metronomo, meu_processador, meu_gravador, meu_gerenciador_jogos):
    ...
    # Barra de Ferramentas Inferior (Layout Flexbox Dinâmico - Borda a Borda)
    num_secoes = len(estado.secoes_inferiores)
    espacamento_entre_botoes = 8 # Gap interno fixo entre os botões
    
    # Cálculo Fluido: Descobre a largura ideal para que o conjunto ocupe EXATAMENTE a largura da box
    # largura_conteudo é o tamanho do dragger (LARGURA_BRACO)
    ...
    for i, secao in enumerate(estado.secoes_inferiores):
        # Posicionamento X incremental: Ocupa todo o espaço interno sem sobras na direita
        x_botao = dx + (i * (largura_botao_fluida + espacamento_entre_botoes))
        rect_cabecalho = pygame.Rect(x_botao, dy, largura_botao_fluida, 45) # Retornado para 45
        ...
        txt = fontes['pequena'].render(txt_traduzido, True, BRANCO) # Retornado para fonte pequena
        ...
        if secao["expandido"]:
            ...
            # Ajuste de Margem e Clipping: O 'y' do rect_clipping sobe 10px para não cortar os títulos
            ...
            if secao["conteudo"] in ["escalas", "acordes"]:
                ...
                if secao["memoria_sub_aba"] < len(chaves):
                    ...
                    for modulo in lista_ativa:
                        if modulo.estado == 'painel':
                            # Sincroniza X e Y com a posição atual do dragger (Permite arrastar a box)
                            x_rel = getattr(modulo, 'x_original_relativo', 0)
                            if not hasattr(modulo, 'x_original_relativo'):
                                # Fallback: se não tiver o relativo, calcula a primeira vez
                ...
                _desenhar_aba_configuracao(tela, dx, y_start, largura_conteudo, estado, fontes, configs, meu_metronomo, secao["memoria_sub_aba"])
                ...
                _desenhar_aba_musicas(tela, dx, y_start, largura_conteudo, estado, fontes, secao["memoria_sub_aba"])
