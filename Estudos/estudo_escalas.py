...
class EstudoEscalas:
    """
        Como funciona: Define a estrutura e estado do componente 'EstudoEscalas'.
        Para que serve: Controla a lógica e interação da tela de estudo prático.
        Onde é usada: Chamado a partir do módulo ou classe base de 'estudo_escalas'.
    """
    ...
    def __init__(self):
        ...
        self.catalogo_shapes = {'Escala Maior - Shape 1': esc_maior.SHAPE_1, 'Escala Maior - Shape 2': esc_maior.SHAPE_2, 'Escala Maior - Shape 3': esc_maior.SHAPE_3, 'Escala Maior - Shape 4': esc_maior.SHAPE_4, 'Escala Maior - Shape 5': esc_maior.SHAPE_5, 'Penta Menor - Shape 1': esc_penta.SHAPE_1, 'Penta Menor - Shape 2': esc_penta.SHAPE_2, 'Penta Menor - Shape 3': esc_penta.SHAPE_3, 'Penta Menor - Shape 4': esc_penta.SHAPE_4, 'Penta Menor - Shape 5': esc_penta.SHAPE_5, 'Blues - Shape 1': esc_avancada.SHAPE_1_BLUES, 'Modo Dórico': esc_modos.DORICO}
        self.notas_base = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
        self.tipos_escala_adivinhar = {'Maior': [0, 2, 4, 5, 7, 9, 11], 'Menor Natural': [0, 2, 3, 5, 7, 8, 10], 'Penta Maior': [0, 2, 4, 7, 9], 'Penta Menor': [0, 3, 5, 7, 10], 'Blues': [0, 3, 5, 6, 7, 10], 'Dórico': [0, 2, 3, 5, 7, 9, 10]}
        self.msg_adivinhar = ''
        ...
        if self.modo_jogo == 'desenhar':
            ...
            for c_matriz in range(len(matriz_shape)):
                ...
                if len(matriz_shape) == 7 and self.num_cordas == 6:
                    ...
                    if c_matriz < len(matriz_shape) - 4:
                        continue
            ...
            if not self.estado_resposta:
                ...
                if self.rect_btn_conferir.collidepoint(pos_mouse_virtual):
                    ...
                    if self.resultado_atual:
                        self.acertos += 1
                    return True
            elif self.rect_btn_conferir.collidepoint(pos_mouse_virtual):
                self.inicializar_questao(estado)
                return True
        elif self.modo_jogo == 'adivinhar':
